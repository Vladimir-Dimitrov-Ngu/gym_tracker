...
def get_gender_keyboard(messages, lang):
    if lang not in messages or 'male' not in messages[lang] or 'female' not in messages[lang] or 'skip' not in messages[lang]:
        raise ValueError("Invalid messages or language")
    keyboard = [
...
def get_muscle_group_keyboard(messages, lang):
    if lang not in messages:
        raise ValueError("Invalid language")
    keyboard = [[InlineKeyboardButton(group, callback_data=f'muscle_{group}')] for group in messages[lang]]
    return InlineKeyboardMarkup(keyboard)

def get_equipment_keyboard(equipment_list, lang):
    if not equipment_list or lang not in WORKOUT_MESSAGES or 'custom_equipment' not in WORKOUT_MESSAGES[lang]:
        raise ValueError("Invalid equipment list or language")
    keyboard = [[InlineKeyboardButton(equip, callback_data=f'equip_{equip}')] for equip in equipment_list]
    keyboard.append([InlineKeyboardButton(WORKOUT_MESSAGES[lang]['custom_equipment'], callback_data='equip_custom')])
    return InlineKeyboardMarkup(keyboard)
